...
fn main() {
    ...
    stdout().flush().unwrap();
    let selected_num: u32 = get_input()
        .trim()
        .parse()
        .expect("Failed tp parse input, please input one of the listed numbers");
...
fn compute_circle_area() {
    ...
    let radius: u32 = get_input()
        .trim()
        .parse()
        .expect("Failed to parse the input, please Input a number");
...
fn solve_quadratic_equation() {
    ...
    if a == 0.0 {
        ...
        match get_quadratic_equation_roots(a, b, c) {
            Some((first_root, second_root)) => println!(
